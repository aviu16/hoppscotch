...
import { isInfraError, resetCage, getCageResetCount } from "~/utils/cage"
...
describe("resetCage and getCageResetCount", () => {
  test("resetCage increments the counter", () => {
    const before = getCageResetCount()
    resetCage()
    expect(getCageResetCount()).toBe(before + 1)
  })

  test("multiple resets increment cumulatively", () => {
    const before = getCageResetCount()
    resetCage()
    resetCage()
    resetCage()
    expect(getCageResetCount()).toBe(before + 3)
  })
})
